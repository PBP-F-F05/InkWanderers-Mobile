...
Future<List<Review>> fetchProduct(CookieRequest request) async {
    ...
    // var url = Uri.parse(
    //     'http://localhost:8000/reviews/get-review/');
    var response = await request.get("http://localhost:8000/reviews/get-review/");
                        ...
                        itemBuilder: (_, index) => InkWell(
                          onTap:  () async {
                            var response = await request.get(
                              'http://localhost:8000/reviews/get-book/${data[index].book.id}'
                            
                            );
                            BookModel.Book bookJson = BookModel.Book.fromJson(response[0]);
                            Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => BookReviewPage(book: bookJson),
                            ),
                          );
                          },
                          child: Container(
                                    ...
                                    ElevatedButton(
                                      onPressed: () {
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                            builder: (context) => EditReviewFormPage(review: data[index]),
                                          ),
                                        );
                                      },
